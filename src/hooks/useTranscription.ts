...
export function useTranscription() {
  ...
  const splitSegment = (
    ...
    setSegments((prev) => {
      ...
      const first: Segment = { ...original, end_sec: splitTime, korean: korFirst, english: engFirst, words: original.words.filter((w) => w.start < splitTime), isTranslating: markTranslating }
      const second: Segment = { ...original, id: newId, start_sec: splitTime, korean: korSecond, english: engSecond, words: original.words.filter((w) => w.start >= splitTime), isTranslating: markTranslating }
