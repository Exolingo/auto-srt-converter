...
function MiniCard({ timeLabel, korean, english, isTranslating }: {
  timeLabel: string
  korean: string
  english: string
...
export function SplitPreviewRow({ segment, split, onConfirm, onCancel }: Props) {
  const { korFirst, korSecond, engFirst, engSecond, splitTime, isTranslating } = split
  ...
  return (
    <div className="flex gap-2 items-start">
      ...
      <div className="flex-1 rounded-xl border-2 border-dashed border-amber-500/70 bg-surface-800 overflow-hidden">
        <div className="px-4 py-2 bg-surface-700 border-b border-surface-600 flex items-center gap-2">
          <span className="text-xs text-amber-400 font-medium">분리 미리보기</span>
          {isTranslating && (
            <span className="text-xs text-slate-500">— 영어 번역 중...</span>
          )}
        </div>
